...
#define SYMBOL_HASH_TABLE_TOTAL_BUCKETS 0x10000
...
LongInt    FwdSymCounter, BackSymCounter;
...
typedef struct sSymbolHashTable {
    PSymbolEntry Buckets[SYMBOL_HASH_TABLE_TOTAL_BUCKETS];
} TSymbolHashTable, *PSymbolHashTable;
...
static TSymbolHashTable FirstSymbol, FirstLocSymbol;
...
static unsigned int GetHash(const char* Name)
{
    ...
    while (*Name != '\0')
        Hash = Hash * 33 + (unsigned int)as_toupper(*Name++); /* Hash the identifier in upper-case form, so that case-insensitive mode works. */

    return Hash;
}

static void SymbolHashTableInit(PSymbolHashTable SymbolHashTable) {
    size_t i;

    for (i = 0; i < SYMBOL_HASH_TABLE_TOTAL_BUCKETS; ++i)
        SymbolHashTable->Buckets[i] = NULL;
}

static void IterSymbolHashTable(PSymbolHashTable SymbolHashTable, TTreeCallback Callback, void* pData) {
    size_t i;

    for (i = 0; i < SYMBOL_HASH_TABLE_TOTAL_BUCKETS; ++i)
        IterTree(&SymbolHashTable->Buckets[i]->Tree, Callback, pData);
}

static void GetSymbolHashTableDepth(PSymbolHashTable SymbolHashTable, LongInt* pMin, LongInt* pMax) {
    size_t i;
    ...
    *pMax = 0;
    ...
    for (i = 0; i < SYMBOL_HASH_TABLE_TOTAL_BUCKETS; ++i)
    {
        const PSymbolEntry Bucket = SymbolHashTable->Buckets[i];
        /* Ignore empty buckets, because they would cause pMin to always be 0. */
        if (Bucket)
            TreeDepthIter(&Bucket->Tree, 0, pMin, pMax);
    }
}

static void DestroySymbolHashTable(PSymbolHashTable SymbolHashTable, TTreeCallback Callback, void* pData) {
    size_t i;

    for (i = 0; i < SYMBOL_HASH_TABLE_TOTAL_BUCKETS; ++i)
    {
        PSymbolEntry* const Bucket = &SymbolHashTable->Buckets[i];
        PTree TreeRoot = &(*Bucket)->Tree;
        *Bucket = NULL;
        DestroyTree(&TreeRoot, Callback, pData);
    }
}

static void DumpSymbolHashTable(PSymbolHashTable SymbolHashTable) {
    size_t i;

    for (i = 0; i < SYMBOL_HASH_TABLE_TOTAL_BUCKETS; ++i)
        DumpTree(&SymbolHashTable->Buckets[i]->Tree);
}

static PSymbolEntry SearchSymbolHashTable(PSymbolHashTable SymbolHashTable, char* Name, LongInt Attribute) {
    return (PSymbolEntry)SearchTree(&SymbolHashTable->Buckets[GetHash(Name) % SYMBOL_HASH_TABLE_TOTAL_BUCKETS]->Tree, Name, Attribute);
}

static Boolean EnterSymbolHashTable(PSymbolHashTable SymbolHashTable, PSymbolEntry Neu, TTreeAdder Adder, void* pData) {
    PSymbolEntry* const Bucket = &SymbolHashTable->Buckets[GetHash(Neu->Tree.Name) % SYMBOL_HASH_TABLE_TOTAL_BUCKETS];
    PTree TreeRoot = &(*Bucket)->Tree;
    const Boolean Result = EnterTree(&TreeRoot, &Neu->Tree, Adder, pData);
    *Bucket = (PSymbolEntry)TreeRoot;
    return Result;
}
...
Boolean RangeCheck(LargeInt Wert, IntType Typ) {
    ...
    if (((int)Typ) >= ((int)SInt64)) {
        ...
        return ((Wert >= IntTypeDefs[(int)Typ].Min)
                && (Wert <= IntTypeDefs[(int)Typ].Max));
    }
}
...
Boolean MultiCharToInt(TempResult* pResult, unsigned MaxLen) {
    if ((pResult->Typ == TempString) && (pResult->Contents.str.len <= MaxLen)
...
Boolean ExpandStrSymbol(char* pDest, size_t DestSize, tStrComp const* pSrc) {
    tStrComp    SrcComp;
    char const* pStart;

    *pDest = '\0';
    StrCompRefRight(&SrcComp, pSrc, 0);
    while (True) {
        ...
        if (pStart) {
            ...
            StrCompCopySub(
                    &ExprComp, &SrcComp, pStart + 1 - SrcComp.str.p_str,
                    pStop - pStart - 1);
...
static Boolean ChkTmp2(char* pDest, char const* pSrc, as_symbol_source_t symbol_source) {
    char const *pRun, *pBegin, *pEnd;
    int         Cnt;
    Boolean     Result = FALSE;
    ...
    */
    ...
    /* forward references ? */
        ...
        if (pRun == pEnd) {
            if ((symbol_source != e_symbol_source_none) && (Cnt == 1)) {
                ...
                as_snprintf(
                        pDest, STRINGSIZE, "__forw%d", (int)(FwdSymCounter + (Cnt - 1)));
                Result = TRUE;
            }
        }
    }
    ...
    /* slash: only allowed for definition, but add to log for backward ref. */
            ...
            (pEnd - pBegin == 1) && (*pBegin == '/')
            && (symbol_source != e_symbol_source_none)) {
...
static Boolean ChkTmp(char* Name, as_symbol_source_t symbol_source) {
    ...
    if (ChkTmp2(Name, Name, symbol_source)) {
        return TRUE;
    }
    if (ChkTmp3(Name, symbol_source)) {
        return TRUE;
    }
    return FALSE;
}

Boolean IdentifySection(tStrComp const* pName, LongInt* Erg) {
    PSaveSection SLauf;
    String       ExpName;
    sint         Depth;
    ...
    if (*ExpName == '\0') {
        *Erg = -1;
        return True;
            ...
            ((strlen(ExpName) == 6) || (strlen(ExpName) == 7))
...
static Boolean GetSymSection(char* Name, LongInt* Erg, tStrComp const* pUnexpComp) {
    String   Part;
    tStrComp TmpComp;
    char*    q;
    int      l = strlen(Name);
...
static LargeInt ConstIntVal(char const* pExpr, IntType Typ, Boolean* pResult) {
    LargeInt              Wert;
    Boolean               NegFlag = False;
    int                   Digit;
    tIntCheckCtx          Ctx;
    tIntFormatList const* pIntFormat;
...
static Double ConstFloatVal(char const* pExpr, FloatType Typ, Boolean* pResult) {
    Double Erg;
    char*  pEnd;
    ...
    if (*pExpr) {
        /* Some strtod() implementations interpret hex constants starting with '0x'.  We
           ...
           have been parsed before by ConstIntVal(), or not, then we don't want the
           constant be stored as float. */

        if ((strlen(pExpr) >= 2) && (pExpr[0] == '0') && (toupper(pExpr[1]) == 'X')) {
            Erg      = 0;
            *pResult = False;
        }
...
static void ConstStringVal(tStrComp const* pExpr, TempResult* pDest, Boolean* pResult) {
    ...
    *pResult = False;
    ...
    while (1) {
        ...
        as_nonz_dynstr_append_raw(
                &pDest->Contents.str, Copy.str.p_str, strlen(Copy.str.p_str));
        ...
        if (*Copy.str.p_str == '{') {
            ...
            case TempInt:
                TLen = SysString(
                        Str, sizeof(Str), t.Contents.Int, OutRadixBase, 0, False,
...
static PSymbolEntry FindLocNode(char const* Name, TempType SearchType);

static PSymbolEntry FindNode(char const* Name, TempType SearchType);
...
void EvalStrExpression(tStrComp const* pExpr, TempResult* pErg) {
    Operator const* pOp;
    Operator const* FOps[OPERATOR_MAXCNT];
    LongInt         FOpCnt = 0;
    ...
    RKlamm = 0;
    WKlamm = 0;
    InSgl = InDbl = ThisEscaped = NextEscaped = False;
    for (pOp = Operators + 1; pOp->Id; pOp++) {
        pOpPos = (pOp->IdLen == 1) ? (strchr(CopyComp.str.p_str, *pOp->Id))
                                   : (strstr(CopyComp.str.p_str, pOp->Id));
        if (pOpPos) {
            FOps[FOpCnt++] = pOp;
        }
    }
    ...
    if (OpMax) {
        ...
        if (!ChkArgCntExtPos(
                    ThisArgCnt, pOp->Dyadic ? 2 : 1, pOp->Dyadic ? 2 : 1,
                    &CopyComp.Pos)) {
            LEAVE;
        }
        ...
        Save = StrCompSplitRef(
                &InArgs[0], &InArgs[1], &CopyComp, CopyComp.str.p_str + OpPos);
        ...
        BestOpMatch     = 255;
        BestOpMatchIdx  = OPERATOR_MAXCOMB;
        SumCombinations = 0;
        for (z = 0; z < OPERATOR_MAXCOMB; z++) {
            ...
            for (z2 = pOp->Dyadic ? 0 : 1; z2 < 2; z2++) {
                ThisOpMatch |= TryConvert(
                        GetOpTypeMask(pOp->TypeCombinations[z], z2), InVals[z2].Typ, z2);
            }
        ...
        if (BestOpMatch >= 255) {
            for (z2 = pOp->Dyadic ? 0 : 1; z2 < 2; z2++) {
                ...
                if (!(TypeMask & InVals[z2].Typ)) {
                    WrStrErrorPos(
                            DeduceExpectTypeErrMsgMask(TypeMask, InVals[z2].Typ),
                            &InArgs[z2]);
                }
            }
            LEAVE;
    ...
    if (LKlamm != 0) {
        ...
        if (ValFunc) {
            ...
            for (z1 = 1; z1 <= ValFunc->ArguCnt; z1++) {
                ...
                if (PromotedDataSize == eSymbolSizeUnknown) {
                    PromotedDataSize = InVals[0].DataSize;
                }
            ...
            if (pErg->DataSize == eSymbolSizeUnknown) {
                pErg->DataSize = PromotedDataSize;
            }
        func_exit2:
        ...
        for (z1 = 0; z1 < cnt; z1++) {
            if ((InVals[z1].Typ == TempInt)
                && (!(pFunction->ArgTypes[z1] & (1 << TempInt)))) {
                TempResultToFloat(&InVals[z1]);
            }
            if (!(pFunction->ArgTypes[z1] & (1 << InVals[z1].Typ))) {
                WrStrErrorPos(
                        DeduceExpectTypeErrMsgMask(
                                pFunction->ArgTypes[z1], InVals[z1].Typ),
                        &InArgs[z1]);
                LEAVE;
            }
        }
...
void EvalExpression(char const* pExpr, TempResult* pErg) {
    tStrComp Expr;
...
LargeInt EvalStrIntExpressionWithResult(
        tStrComp const* pComp, IntType Type, tEvalResult* pResult) {
    TempResult t;
    LargeInt   Result = -1;
...
LargeInt EvalStrIntExpressionWithFlags(
        tStrComp const* pComp, IntType Type, Boolean* pResult, tSymbolFlags* pFlags) {
    tEvalResult EvalResult;
...
LargeInt EvalStrIntExpression(tStrComp const* pComp, IntType Type, Boolean* pResult) {
    tEvalResult EvalResult;
    LargeInt    Result = EvalStrIntExpressionWithResult(pComp, Type, &EvalResult);
...
LargeInt EvalStrIntExpressionOffsWithResult(
        tStrComp const* pExpr, int Offset, IntType Type, tEvalResult* pResult) {
    if (Offset) {
        tStrComp Comp;
...
LargeInt EvalStrIntExpressionOffsWithFlags(
        tStrComp const* pComp, int Offset, IntType Type, Boolean* pResult,
        tSymbolFlags* pFlags) {
    tEvalResult EvalResult;
    LargeInt    Result
            = EvalStrIntExpressionOffsWithResult(pComp, Offset, Type, &EvalResult);
...
LargeInt EvalStrIntExpressionOffs(
        tStrComp const* pComp, int Offset, IntType Type, Boolean* pResult) {
    tEvalResult EvalResult;
    LargeInt    Result
            = EvalStrIntExpressionOffsWithResult(pComp, Offset, Type, &EvalResult);
...
Double EvalStrFloatExpressionWithResult(
        tStrComp const* pExpr, FloatType Type, tEvalResult* pResult) {
    TempResult t;
    Double     Result = -1;
...
Double EvalStrFloatExpression(tStrComp const* pExpr, FloatType Type, Boolean* pResult) {
    Double      Ret;
    tEvalResult Result;
...
void EvalStrStringExpressionWithResult(
        tStrComp const* pExpr, tEvalResult* pResult, char* pEvalResult) {
    TempResult t;
...
void EvalStrStringExpression(tStrComp const* pExpr, Boolean* pResult, char* pEvalResult) {
    tEvalResult Result;
...
tErrorNum EvalStrRegExpressionWithResult(
        const struct sStrComp* pExpr, tRegDescr* pResult, tEvalResult* pEvalResult) {
    PSymbolEntry pEntry;
...
/*!------------------------------------------------------------------------
 * \fn     EvalStrRegExpressionAsOperand(const tStrComp *pArg, struct sRegDescr *pResult,
 struct sEvalResult *pEvalResult, tSymbolSize ReqSize, Boolean MustBeReg)
...
tRegEvalResult EvalStrRegExpressionAsOperand(
        tStrComp const* pArg, struct sRegDescr* pResult, struct sEvalResult* pEvalResult,
        tSymbolSize ReqSize, Boolean MustBeReg) {
    tErrorNum ErrorNum;

    ErrorNum = EvalStrRegExpressionWithResult(pArg, pResult, pEvalResult);
    if (pEvalResult->OK && (ReqSize != eSymbolSizeUnknown)
...
static Boolean SymbolAdder(PTree* PDest, PTree Neu, void* pData) {
    ...
    if (((*Node)->Defined) && (!(*Node)->Changeable) && (!EnterStruct->MayChange)) {
        ...
        if (EnterStruct->DoCross) {
            as_snprcatf(
                    serr, STRINGSIZE, ",%s %s:%ld", getmessage(Num_PrevDefMsg),
    ...
    /* tried to reassign a constant (EQU) a value with SET and vice versa ? */
        ...
        WrXError(
                (*Node)->Changeable ? ErrNum_VariableRedefinedAsConstant
                                    : ErrNum_ConstantRedefinedAsVariable,
                serr);
        FreeSymbolEntry(&NewEntry, TRUE);
        return False;
    }
        ...
        if (!EnterStruct->MayChange) {
            ...
            if ((NewEntry->SymWert.Typ != (*Node)->SymWert.Typ)
                || ((NewEntry->SymWert.Typ == TempString)
                    && (as_nonz_dynstr_cmp(
                            &NewEntry->SymWert.Contents.str,
                            &(*Node)->SymWert.Contents.str)))
                || ((NewEntry->SymWert.Typ == TempFloat)
                    && (NewEntry->SymWert.Contents.Float
                        != (*Node)->SymWert.Contents.Float))
                || ((NewEntry->SymWert.Typ == TempInt)
                    && (NewEntry->SymWert.Contents.Int
                        != (*Node)->SymWert.Contents.Int))) {
...
static void EnterSymbol_Search(
        PForwardSymbol* Lauf, PForwardSymbol* Prev, PForwardSymbol** RRoot,
        PSymbolEntry Neu, PForwardSymbol* Root, Byte ResCode, Byte* SearchErg) {
    *Lauf  = (*Root);
    *Prev  = NULL;
    *RRoot = Root;
...
static void EnterSymbol(PSymbolEntry Neu, Boolean MayChange, LongInt ResHandle) {
    ...
    if ((SectionStack) && (Neu->Tree.Attribute == MomSectionHandle)) {
        EnterSymbol_Search(
                &Lauf, &Prev, &RRoot, Neu, &(SectionStack->LocSyms), 1, &SearchErg);
        if (!Lauf) {
            EnterSymbol_Search(
                    &Lauf, &Prev, &RRoot, Neu, &(SectionStack->GlobSyms), 2, &SearchErg);
        }
        if (!Lauf) {
            EnterSymbol_Search(
                    &Lauf, &Prev, &RRoot, Neu, &(SectionStack->ExportSyms), 3,
                    &SearchErg);
        }
        if (SearchErg == 2) {
...
PSymbolEntry CreateSymbolEntry(
        tStrComp const* pName, LongInt* pDestHandle, tSymbolFlags symbol_flags) {
    PSymbolEntry pNeu;
    String       ExtName;
    ...
    (void)ChkTmp(
            ExtName, (symbol_flags & eSymbolFlag_Label) ? e_symbol_source_label
...
PSymbolEntry EnterIntSymbolWithFlags(
        tStrComp const* pName, LargeInt Wert, as_addrspace_t addrspace, Boolean MayChange,
        tSymbolFlags Flags) {
    LongInt      DestHandle;
...
void EnterExtSymbol(
        tStrComp const* pName, LargeInt Wert, as_addrspace_t addrspace,
        Boolean MayChange) {
    LongInt      DestHandle;
...
void EnterFloatSymbol(tStrComp const* pName, Double Wert, Boolean MayChange) {
    LongInt      DestHandle;
    PSymbolEntry pNeu = CreateSymbolEntry(pName, &DestHandle, eSymbolFlag_None);
...
void EnterNonZStringSymbolWithFlags(
        tStrComp const* pName, as_nonz_dynstr_t const* p_value, Boolean MayChange,
        tSymbolFlags Flags) {
    LongInt      DestHandle;
    PSymbolEntry pNeu = CreateSymbolEntry(pName, &DestHandle, Flags);
    ...
    pNeu->SymWert.Contents.str.len = pNeu->SymWert.Contents.str.capacity = p_value->len;
...
void EnterStringSymbol(tStrComp const* pName, char const* pValue, Boolean MayChange) {
...
void EnterRegSymbol(
        const struct sStrComp* pName, tRegDescr const* pDescr, tSymbolSize Size,
        Boolean MayChange, Boolean AddList) {
    LongInt      DestHandle;
    PSymbolEntry pNeu = CreateSymbolEntry(pName, &DestHandle, eSymbolFlag_None);
...
static PSymbolEntry FindNode(char const* Name_O, TempType SearchType) {
    PSaveSection Lauf;
    LongInt      DestSection;
    ...
    if (SectionStack) {
        if (PassNo <= MaxSymPass) {
            if (FindNode_FSpec(Name, SectionStack->LocSyms)) {
...
static PSymbolEntry FindLocNode(char const* Name_O, TempType SearchType) {
...
void LookupSymbol(
        const struct sStrComp* pComp, TempResult* pValue, Boolean WantRelocs,
        TempType ReqType) {
    PSymbolEntry pEntry;
    String       ExpName;
    char const*  pKlPos;
    ...
    if (pEntry) {
        ...
        if ((pEntry->SymWert.DataSize != eSymbolSizeUnknown)
            && (pValue->DataSize == eSymbolSizeUnknown)) {
...
typedef struct {
    ...
    int         ZeilenrestLen, ZeilenrestVisibleLen;
    as_dynstr_t s1, sh;
} TListContext;

static void PrintSymbolList_AddOut(char* s, TListContext* pContext) {
    int AddVisibleLen = visible_strlen(s), AddLen = strlen(s);
...
static void PrintSymbolList_PNode(PTree Tree, void* pData) {
    ...
    if (Node->SymWert.Typ != TempReg) {
        ...
        int               l1, nBlanks;
        TempResult const* pValue = &Node->SymWert;

        if ((pValue->Typ == TempInt) && DissectBit
            && (pValue->AddrSpaceMask & (1 << SegBData))) {
        ...
        for (nBlanks = pContext->cwidth - 1 - l1; nBlanks < 0;
             nBlanks += pContext->cwidth)
            ;
        as_sdprcatf(
                &pContext->sh, "%s%s %c | ", Blanks(nBlanks), pContext->s1.p_str,
                SegShorts[addrspace_from_mask(pValue->AddrSpaceMask)]);
...
void PrintSymbolList(void) {
    ...
    Context.ZeilenrestLen = Context.ZeilenrestVisibleLen = 0;
    ...
    WrLstLine("");
    as_sdprintf(
            &Context.Zeilenrest, "%7lu%s", (unsigned long)Context.Sum,
            getmessage((Context.Sum == 1) ? Num_ListSymSumMsg : Num_ListSymSumsMsg));
    WrLstLine(Context.Zeilenrest.p_str);
    as_sdprintf(
            &Context.Zeilenrest, "%7lu%s", (unsigned long)Context.USum,
...
static void PrintDebSymbols_PNode(PTree Tree, void* pData) {
    ...
    if (Node->SymWert.Typ == TempString) {
        errno = 0;
        l1    = fstrlenprint(
                DebContext->f, Node->SymWert.Contents.str.p_str,
                Node->SymWert.Contents.str.len);
        ChkIO(ErrNum_FileWriteError);
    } else {
    ...
    fprintf(DebContext->f, "%s %-3d %d %d\n", Blanks(25 - l1), Node->SymWert.DataSize,
            (int)Node->Used, (int)Node->Changeable);
    ChkIO(ErrNum_FileWriteError);
}
...
static void PrNoISection(PTree Tree, void* pData) {
    ...
    if ((Node->SymWert.AddrSpaceMask & NoICEMask)
        && (Node->Tree.Attribute == pContext->Handle) && (Node->SymWert.Typ == TempInt)) {
        errno = 0;
        fprintf(pContext->f, "DEFINE %s 0x", Node->Tree.Name);
        ChkIO(ErrNum_FileWriteError);
        errno = 0;
        fprintf(pContext->f, "%" PRIx64, (LargeWord)Node->SymWert.Contents.Int);
        ChkIO(ErrNum_FileWriteError);
        errno = 0;
        fprintf(pContext->f, "\n");
        ChkIO(ErrNum_FileWriteError);
...
void PrintNoISymbols(FILE* f) {
    ...
    for (CurrSection = FirstSection; CurrSection; CurrSection = CurrSection->Next) {
        if (ChunkSum(&CurrSection->Usage) > 0) {
            fprintf(f, "FUNCTION %s ", CurrSection->Name);
            ChkIO(ErrNum_FileWriteError);
            fprintf(f, "%" PRIu64, ChunkMin(&CurrSection->Usage));
            ChkIO(ErrNum_FileWriteError);
            fprintf(f, "\n");
            ChkIO(ErrNum_FileWriteError);
            IterSymbolHashTable(&FirstSymbol, PrNoISection, &Context);
            ...
            fprintf(f, "}FUNC ");
            ChkIO(ErrNum_FileWriteError);
            fprintf(f, "%" PRIu64, ChunkMax(&CurrSection->Usage));
            ChkIO(ErrNum_FileWriteError);
            fprintf(f, "\n");
            ChkIO(ErrNum_FileWriteError);
        }
...
Boolean PushSymbol(tStrComp const* pSymName, tStrComp const* pStackName) {
    PSymbolEntry      pSrc;
...
Boolean PopSymbol(tStrComp const* pSymName, tStrComp const* pStackName) {
    PSymbolEntry      pDest;
...
void EnterFunction(tStrComp const* pComp, char* FDefinition, Byte NewCnt) {
    PFunction   Neu;
    String      FName_N;
    char const* pFName;
...
PFunction FindFunction(char const* Name) {
...
TempResult const* FindDefSymbol(char const* pName) {
    PDefSymbol pRun;
...
char const* GetSectionName(LongInt Handle) {
...
void PrintDebSections(FILE* f) {
    ...
    while (Lauf) {
        fputs("\nInfo for Section ", f);
        ChkIO(ErrNum_FileWriteError);
        fprintf(f, "%" PRId32, Cnt);
        ChkIO(ErrNum_FileWriteError);
        fputc(' ', f);
        ChkIO(ErrNum_FileWriteError);
        fputs(GetSectionName(Cnt), f);
        ChkIO(ErrNum_FileWriteError);
        fputc(' ', f);
        ChkIO(ErrNum_FileWriteError);
        fprintf(f, "%" PRId32, Lauf->Parent);
        ChkIO(ErrNum_FileWriteError);
        fputc('\n', f);
        ChkIO(ErrNum_FileWriteError);
...
static void PrintCrossList_PNode(PTree Node, void* pData) {
    ...
    as_snprintf(h, sizeof(h), "%s%s", getmessage(Num_ListCrossSymName), Node->Name);
    if (Node->Attribute != -1) {
        as_snprcatf(h, sizeof(h), "[%s]", GetSectionName(Node->Attribute));
    }
    as_snprcatf(
            h, sizeof(h), " (=%s, %s:%s):", p_val_str->p_str,
            GetFileName(SymbolEntry->FileNum), LineStr);
...
static void PrintRegList_PNode(PTree Tree, void* pData) {
    ...
    if (Node->SymWert.Typ == TempReg) {
        ...
        if (Node->SymWert.Contents.RegDescr.Dissect) {
            Node->SymWert.Contents.RegDescr.Dissect(
                    tmp2, sizeof(tmp2), Node->SymWert.Contents.RegDescr.Reg,
                    Node->SymWert.DataSize);
        } else {
            *tmp2 = '\0';
        }
        *tmp = '\0';
        ...
        as_snprcatf(
                tmp, sizeof(tmp), "%c%s --> %s", Node->Used ? ' ' : '*', Tree->Name,
                tmp2);
...
void PrintRegDefs(void) {
    ...
    WrLstLine("");
    as_snprintf(
            buf, sizeof(buf), "%7ld%s", (long)Context.Sum,
            getmessage(
                    (Context.Sum == 1) ? Num_ListRegDefSumMsg : Num_ListRegDefSumsMsg));
    WrLstLine(buf);
    as_snprintf(
            buf, sizeof(buf), "%7ld%s", (long)Context.USum,
            getmessage(
                    (Context.USum == 1) ? Num_ListRegDefUSumMsg
                                        : Num_ListRegDefUSumsMsg));
    WrLstLine("");
    as_dynstr_free(&Context.Zeilenrest);
    as_dynstr_free(&Context.s1);
    as_dynstr_free(&Context.sh);
...
void PrintCodepages(void) {
    ...
    for (Table = TransTables; Table; Table = Table->Next) {
        ...
        as_snprintf(
                buf, sizeof(buf), "%s (%d%s)", Table->Name, cnt,
                getmessage(
                        (cnt == 1) ? Num_ListCodepageChange : Num_ListCodepagePChange));
        WrLstLine(buf);
        cnt2++;
    }
    WrLstLine("");
    as_snprintf(
            buf, sizeof(buf), "%d%s", cnt2,
...
void asmpars_init(void) {
    ...
    for (pCurr = IntTypeDefs;
         pCurr < IntTypeDefs + (sizeof(IntTypeDefs) / sizeof(*IntTypeDefs)); pCurr++) {
