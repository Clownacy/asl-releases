...
static ShortInt StrCmp(char const* s1, char const* s2, LongInt Hand1, LongInt Hand2) {
    int tmp;
...
void IterTree(PTree Tree, TTreeCallback Callback, void* pData) {
    if (Tree) {
        if (Tree->Left) {
            IterTree(Tree->Left, Callback, pData);
        }
        Callback(Tree, pData);
        if (Tree->Right) {
            IterTree(Tree->Right, Callback, pData);
        }
    }
}
...
void DestroyTree(PTree* Tree, TTreeCallback Callback, void* pData) {
    if (*Tree) {
        if ((*Tree)->Left) {
            DestroyTree(&((*Tree)->Left), Callback, pData);
        }
        if ((*Tree)->Right) {
            DestroyTree(&((*Tree)->Right), Callback, pData);
        }
        Callback(*Tree, pData);
        if ((*Tree)->Name) {
            free((*Tree)->Name);
...
static void DumpTreeIter(PTree Tree, LongInt Level) {
    if (Tree) {
        if (Tree->Left) {
            DumpTreeIter(Tree->Left, Level + 1);
        }
        fprintf(Debug, "%*s%s\n", 6 * Level, "", Tree->Name);
        if (Tree->Right) {
            DumpTreeIter(Tree->Right, Level + 1);
        }
    }
}
